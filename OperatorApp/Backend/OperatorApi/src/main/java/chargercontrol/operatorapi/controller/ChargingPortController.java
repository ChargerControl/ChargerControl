...
public class ChargingPortController {
    ...
    @Operation(summary = "Get a charging port by its ID", responses = {
            @ApiResponse(responseCode = "200", description = "Charging port retrieved successfully", 
                        content = @Content(mediaType = "application/json", schema = @Schema(implementation = ChargingPort.class))),
    ...
    @Operation(summary = "Update the status of a charging port", responses = {
            @ApiResponse(responseCode = "200", description = "Charging port status updated successfully", 
                        content = @Content(mediaType = "application/json", schema = @Schema(implementation = ChargingPort.class))),
            @ApiResponse(responseCode = "400", description = "Invalid status value"),
            @ApiResponse(responseCode = "404", description = "Charging port not found")
    })
    public ResponseEntity<ChargingPort> updateChargingPortStatus(
            ...
            @RequestBody Map<String, ChargingPortStatus> statusUpdate) {
        try {
            ChargingPortStatus status = statusUpdate.get("status");
            if (status == null) {
    ...
    @PutMapping("/{portId}/energy")
    @Operation(summary = "Update the energy used by a charging port", responses = {
            @ApiResponse(responseCode = "200", description = "Charging port energy updated successfully", 
                        content = @Content(mediaType = "application/json", schema = @Schema(implementation = ChargingPort.class))),
            @ApiResponse(responseCode = "400", description = "Invalid energy value"),
            @ApiResponse(responseCode = "404", description = "Charging port not found")
    })
    public ResponseEntity<ChargingPort> updateChargingPortEnergyUsed(
            @Parameter(description = "ID of the charging port to update") @PathVariable Long portId,
            @RequestBody Map<String, Double> energyUpdate) {
        try {
            Double energyUsed = energyUpdate.get("energyUsed");
            if (energyUsed == null || energyUsed < 0) {
                return ResponseEntity.badRequest().build();
            }
            ChargingPort updatedPort = chargingPortService.updateChargingPortEnergyUsed(portId, energyUsed);
            return ResponseEntity.ok(updatedPort);
        } catch (RuntimeException e) {
            return ResponseEntity.notFound().build();
        }
    ...
    @PatchMapping("/{portId}/energy/increment")
    @Operation(summary = "Increment the energy used by a charging port", responses = {
            @ApiResponse(responseCode = "200", description = "Charging port energy incremented successfully", 
                        content = @Content(mediaType = "application/json", schema = @Schema(implementation = ChargingPort.class))),
            @ApiResponse(responseCode = "400", description = "Invalid energy value"),
            @ApiResponse(responseCode = "404", description = "Charging port not found")
    })
    public ResponseEntity<ChargingPort> incrementEnergyUsed(
            @Parameter(description = "ID of the charging port to update") @PathVariable Long portId,
            @RequestBody Map<String, Double> energyUpdate) {
        try {
            Double additionalEnergy = energyUpdate.get("additionalEnergy");
            if (additionalEnergy == null || additionalEnergy < 0) {
                return ResponseEntity.badRequest().build();
            }
            ChargingPort updatedPort = chargingPortService.incrementEnergyUsed(portId, additionalEnergy);
            return ResponseEntity.ok(updatedPort);
        } catch (RuntimeException e) {
            return ResponseEntity.notFound().build();
        }
    ...
    @PatchMapping("/{portId}/energy/reset")
    @Operation(summary = "Reset the energy used by a charging port to zero", responses = {
            @ApiResponse(responseCode = "200", description = "Charging port energy reset successfully", 
                        content = @Content(mediaType = "application/json", schema = @Schema(implementation = ChargingPort.class))),
            @ApiResponse(responseCode = "404", description = "Charging port not found")
    })
    public ResponseEntity<ChargingPort> resetEnergyUsed(
            @Parameter(description = "ID of the charging port to reset") @PathVariable Long portId) {
        try {
            ChargingPort updatedPort = chargingPortService.resetEnergyUsed(portId);
            return ResponseEntity.ok(updatedPort);
        } catch (RuntimeException e) {
            return ResponseEntity.notFound().build();
        }
    ...
    @Operation(summary = "Update an existing charging port", responses = {
            @ApiResponse(responseCode = "200", description = "Charging port updated successfully", 
                        content = @Content(mediaType = "application/json", schema = @Schema(implementation = ChargingPort.class))),
            @ApiResponse(responseCode = "400", description = "Invalid charging port data"),
            @ApiResponse(responseCode = "404", description = "Charging port not found")
    })
    ...
    @GetMapping("/station/{stationId}/stats/available")
    @Operation(summary = "Get count of available charging ports for a station", responses = {
            @ApiResponse(responseCode = "200", description = "Count retrieved successfully"),
            @ApiResponse(responseCode = "404", description = "Station not found")
    })
    public ResponseEntity<Map<String, Long>> getAvailablePortsCount(
            @Parameter(description = "ID of the station") @PathVariable Long stationId) {
        try {
            long count = chargingPortService.countAvailablePortsByStation(stationId);
            return ResponseEntity.ok(Map.of("availablePorts", count));
        } catch (RuntimeException e) {
            return ResponseEntity.notFound().build();
        }
    }

    @GetMapping("/station/{stationId}/stats/charging")
    @Operation(summary = "Get count of charging ports currently in use for a station", responses = {
            @ApiResponse(responseCode = "200", description = "Count retrieved successfully"),
            @ApiResponse(responseCode = "404", description = "Station not found")
    })
    public ResponseEntity<Map<String, Long>> getChargingPortsCount(
            @Parameter(description = "ID of the station") @PathVariable Long stationId) {
        try {
            long count = chargingPortService.countChargingPortsByStation(stationId);
            return ResponseEntity.ok(Map.of("chargingPorts", count));
        } catch (RuntimeException e) {
            return ResponseEntity.notFound().build();
        }
    }
